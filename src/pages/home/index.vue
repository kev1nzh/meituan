    ...
    <div class="content">
      ...
      >
        <block v-for="(item, index) in topBannerData" :key="index">
          <swiper-item>
            <img class="ad-img" :src="item.banner" />
          </swiper-item>
        </block>
      </swiper>
        ...
        <div v-for="(item, index) in categoryArr" :key="index">
          <div class="grid-c">
            <div
              ...
              @click="categoryClick"
            >
              <img class="item-img" :src="itx.banner" />
              <span class="item-title">{{ itx.title }}</span>
            </div>
          </div>
        </div>
      </div>
              ...
              <div class="r-t">
                ...
                <div class="t-c">
                  <div class="c-l">
                    ...
                    <span class="l-m">{{ item.avg_score }}</span>
                    <span class="l-r">已售{{ item.orders }}</span>
                  </div>
                  <div class="c-r">
                    <span class="r-l">{{ item.pei_time }}分钟</span>

                    <span class="r-r">{{ item.juli }}m</span>
                  </div>
                </div>
              </div>
              <div class="r-m">
                <div>
                  <span class="m-l">起送￥{{ item.min_amount }}</span>

                  <span class="m-r">配送￥{{ item.freight }}</span>
                </div>
                <div class="m-d">
                  <span>{{ item && item.peiType && item.peiType.label }}</span>
                </div>
              </div>
              <div class="r-b">
...
import Dialog from "../../../static/wxcomponents/vant/dialog/dialog";
import Toast from "../../../static/wxcomponents/vant/toast/toast";
export default {
  data() {
    return {
  ...
  methods: {
    ...
    getUserInfo(event) {
      wx.setStorageSync("userDetail", JSON.stringify(event.mp.detail));
      this.getToken();
    },
    async getToken() {
      const { code } = await this.getLogin();
      const { encryptedData, iv, userInfo } = JSON.parse(
      ...
      console.log(httpObj);
      const result = await this.post("client/wxapp/endata", httpObj);
      console.log(result);
    },
    async init() {
      wx.showLoading({
      ...
      const result = await this.post("client/index", httpObj);

      console.log(result.data.data.config, result.data.data.waimai);
      this.mainProductList = result.data.data.config[2].content;
      this.topBannerData = result.data.data.config[3].content;
      this.categoryArr[0].items = result.data.data.config[4].content;
      this.shopsList = result.data.data.waimai.items;
      this.searchTitle = result.data.data.config[1].so_title;
      wx.hideLoading();
    },
  },
  mounted() {
